...
- Python dependencies: pycountry==18.2.23 PyYAML==5.4.1 requests==2.18.4
...
- Script takes two args:
    path to `library.xml` file and
    final wanted path for `catalog.yml` file.
- Script first writes to a temporary file and only creates final yaml file
    upon success so it's safe to use the final location.
- Script **does not download** `library.xml` file.
    You can easily chain it to a `wget`|`curl` command though.
"""
...
with_size = False  # will use library's * 1024
mirrorbrain_url = os.getenv("MIRRORBRAIN_URL", False)


def get_remote_checksum(url):
    """retrieve SHA-256 checksum of download using special mirrorbrain url"""
    if mirrorbrain_url:
        url = url.replace("https://download.kiwix.org", mirrorbrain_url)
...
def get_kind(book):
    ...
    return re.sub(r"\.zim\.meta4$", "", re.sub(r"_20[0-9]{2}\-[0-1][0-9]", "", fname))
...
def convert(library_fpath, catalog_fpath, local_repository=False):
    ...
    logger.info(
        "starting convertion of `{}` to `{}`".format(library_fpath, catalog_fpath)
    )
...
if __name__ == "__main__":
    if not len(sys.argv[1:]) == 2:
        logger.error("Usage: {} library.xml catalog.yml".format(sys.argv[0]))
        sys.exit(1)
